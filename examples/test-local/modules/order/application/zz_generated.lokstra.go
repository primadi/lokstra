...
import (
	...
	domain "examples/test-local/modules/order/domain"
...
func OrderServiceImplFactory(deps map[string]any, config map[string]any) any {
	svc := &OrderServiceImpl{
		OrderRepo:   deps["@repository.order-repository"].(domain.OrderRepository),
...
func RegisterOrderServiceImpl() {
	...
	lokstra_registry.RegisterLazyService("order-service",
		...
		map[string]any{
			"depends-on": []string{"@repository.order-repository", "user-service"},
		})
}
