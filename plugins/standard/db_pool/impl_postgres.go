...
// InstanceName implements core.Service.
func (p *pgxPostgresPool) InstanceName() string {
...
func newPgxPostgresPool(instanceName string, dsn string) (*pgxPostgresPool, error) {
	...
	return &pgxPostgresPool{instanceName: instanceName, dsn: dsn, pool: pool}, nil
}

func (p *pgxPostgresPool) Acquire(schema string) (DBConn, error) {
...
func (c *pgxConnWrapper) Exec(ctx context.Context, query string, args ...any) (CommandResult, error) {
	tag, err := c.conn.Exec(ctx, query, args...)
	return &pgxCommandResult{fnRowsAffected: tag.RowsAffected}, err
}

func (c *pgxConnWrapper) Query(ctx context.Context, query string, args ...any) (RowIterator, error) {
...
func (c *pgxConnWrapper) QueryRow(ctx context.Context, query string, args ...any) RowScanner {
...
func pgxSelectManyWithMapper(ctx context.Context,
	conn dbExecutor, fnScan func(RowScanner) (any, error), query string, args ...any) (any, error) {
	rows, err := conn.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("failed to execute query: %w", err)
	}
...
func (c *pgxConnWrapper) Begin(ctx context.Context) (Tx, error) {
...
func (c *pgxConnWrapper) Transaction(ctx context.Context, fn func(tx DBConn) error) error {
	tx, err := c.conn.Begin(ctx)
	if err != nil {
		return err
	}
