...
func TestParseMiddlewareName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			name, params := internal.ParseMiddlewareName(tt.input)

			if name != tt.expectedName {
				t.Errorf("internal.ParseMiddlewareName(%q) name = %q, want %q", tt.input, name, tt.expectedName)
			}

			if !reflect.DeepEqual(params, tt.expectedParams) {
				t.Errorf("internal.ParseMiddlewareName(%q) params = %v, want %v", tt.input, params, tt.expectedParams)
			}
		})
	}
}
...
func TestMergeConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := internal.MergeConfig(tt.base, tt.override)

			if !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("internal.MergeConfig(%v, %v) = %v, want %v", tt.base, tt.override, result, tt.expected)
			}
		})
	}
}
