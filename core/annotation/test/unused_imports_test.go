...
`
	...
	// Should include deploy and proxy for @Handler
	if !strings.Contains(genCode, `"github.com/primadi/lokstra/core/deploy"`) {
		t.Error("Generated code should import deploy for @Handler")
	}

	if !strings.Contains(genCode, `"github.com/primadi/lokstra/core/proxy"`) {
		t.Error("Generated code should import proxy for @Handler")
	}
