...
func TestLazyLoadFromConfig(t *testing.T) {
	_ = deploy.Global()

	testSvc := &TestService{Name: "config-test"}
	lokstra_registry.RegisterService("db-service", testSvc)
	...
	lazy := service.LazyLoadFromConfig[*TestService](config, "database")
	if lazy == nil {
		t.Fatal("expected lazy loader to be created")
	}

	retrieved := lazy.Get()
	if retrieved.Name != "config-test" {
		t.Errorf("expected name 'config-test', got '%s'", retrieved.Name)
	}
}

func TestLazyLoadFromConfig_MissingKey(t *testing.T) {
	config := map[string]any{
		"timeout": 30,
	}
	...
	lazy := service.LazyLoadFromConfig[*TestService](config, "database")
	if lazy != nil {
		t.Error("expected nil when key is missing")
	}
}

func TestMustLazyLoadFromConfig_Success(t *testing.T) {
	_ = deploy.Global()

	testSvc := &TestService{Name: "must-config"}
	lokstra_registry.RegisterService("cache-service", testSvc)

	config := map[string]any{
		"cache": "cache-service",
	}

	lazy := service.MustLazyLoadFromConfig[*TestService](config, "cache")
	retrieved := lazy.Get()

	if retrieved.Name != "must-config" {
		t.Errorf("expected name 'must-config', got '%s'", retrieved.Name)
	}
}

func TestMustLazyLoadFromConfig_Panic(t *testing.T) {
	config := map[string]any{
		"timeout": 30,
	}
	...
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected panic when required key is missing")
		}
	}()

	service.MustLazyLoadFromConfig[*TestService](config, "database")
}
