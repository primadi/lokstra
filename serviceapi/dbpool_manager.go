...
type DbPoolManager interface {
	...
	GetAllDbPoolManager() map[string]*DbPoolInfo
	...
	SetDbPoolManager(name string, dsn string, schema string, rlsContext map[string]string)
	// get dsn, schema, rlsContext for the given name
	GetDbPoolManagerInfo(name string) (string, string, map[string]string, error)
	// get DbPool for the given name
	GetDbPoolManager(name string) (DbPool, error)
	// remove name mapping
	RemoveDbPoolManager(name string)
