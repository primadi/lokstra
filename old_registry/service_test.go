...
func TestRegisterAndGetService(t *testing.T) {
	t.Run("register and get by concrete type", func(t *testing.T) {
		old_registry.RegisterService("serviceA", newServiceA("Service A"),
			old_registry.AllowOverride(true))
		svcA := old_registry.GetService[*serviceA]("serviceA")
		if svcA == nil {
	...
	t.Run("get by interface type", func(t *testing.T) {
		namedSvc := old_registry.GetService[namedService]("serviceA")
		if namedSvc == nil {
	...
	t.Run("try get with wrong interface", func(t *testing.T) {
		_, ok := old_registry.TryGetService[descService]("serviceA")
		if ok {
			t.Errorf("serviceA should not be found with DescService interface")
		}
		_, ok = old_registry.TryGetService[descService]("serviceC")
		if ok {
...
func TestNewService(t *testing.T) {
	t.Run("register factory and create service", func(t *testing.T) {
		old_registry.RegisterServiceType("typeA", func(config map[string]any) any {
			return &serviceA{name: config["name"].(string)}
		}, old_registry.AllowOverride(true))
		svcA := old_registry.NewService[namedService]("newServiceA", "typeA", map[string]any{"name": "Service A"})
		if svcA == nil {
			t.Fatalf("Failed to create serviceA")
		}
		var newServiceA *serviceA
		var ok bool
		if newServiceA, ok = old_registry.TryGetService[*serviceA]("newServiceA"); !ok {
	...
	t.Run("create service with unregistered factory", func(t *testing.T) {
		svcB := old_registry.NewService[*serviceA]("newServiceB", "typeB", map[string]any{"name": "Service B"})
		if svcB != nil {
...
func TestGetService_PanicNotFound(t *testing.T) {
	...
	_ = old_registry.MustGetService[*serviceA]("notExistService")
}
...
func TestGetService_PanicTypeMismatch(t *testing.T) {
	...
	_ = old_registry.GetService[descService]("serviceA")
}

func TestRegisterLazyServiceAndGetService(t *testing.T) {
	// Register factory for lazy type
	old_registry.RegisterServiceType("lazyTypeA", func(config map[string]any) any {
		return &serviceA{name: config["name"].(string)}
	})

	// Register lazy service
	old_registry.RegisterLazyService("lazyServiceA", "lazyTypeA",
		map[string]any{"name": "Lazy Service A"})

	t.Run("GetService from lazy service", func(t *testing.T) {
		svcA := old_registry.GetService[*serviceA]("lazyServiceA")
		if svcA == nil {
	...
	t.Run("GetService with wrong type should panic", func(t *testing.T) {
		...
		_ = old_registry.MustGetService[descService]("lazyServiceA")
	})
}
