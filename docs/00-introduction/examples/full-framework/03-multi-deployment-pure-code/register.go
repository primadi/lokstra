...
import (
	...
	"github.com/primadi/lokstra/docs/00-introduction/examples/full-framework/03-multi-deployment-pure-code/service"
...
func beforeAfterLoggerFactory() request.HandlerFunc {
	return func(ctx *request.Context) error {
		...
		logger.LogInfo("Starting request id=%s %s %s\n", reqId, ctx.R.Method, ctx.R.URL.Path)
		...
		// 3. After request
		if err != nil {
			logger.LogError("Request id=%s failed: %v\n", reqId, err)
		} else {
			logger.LogInfo("Request id=%s completed successfully\n", reqId)
		}
		return err
	}
}
